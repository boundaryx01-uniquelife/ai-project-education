...
$Checks = @(
    ...
    @{ Name = "Level 2 web-tool package export"; Value = $Workshop.Contains('downloadMarkdown') -and $Workshop.Contains('downloadHtml') -and -not $Workshop.Contains('downloadHandoff') },
    ...
    @{ Name = "Double-click local launcher"; Value = (Get-Content -LiteralPath $EasyLauncher -Raw).Contains('Start-LocalPortal.ps1') }
)
...
Write-Host "PASS: LEVEL 2 deployable web-tool static validation"
