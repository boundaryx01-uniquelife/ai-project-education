...
const stages=[
  {name:"1. MVP 제작",title:"AI에게 실제로 실행되는 첫 MVP를 만들게 한다",web:"목표와 최소 기능을 정합니다. 그 뒤 AI가 만든 단일 HTML 코드를 아래에 붙여넣으면 왼쪽에서 즉시 실행됩니다.",ai:"설명서나 MD가 아니라 HTML/CSS/JS가 모두 포함된 단일 실행 파일을 만들도록 요청합니다.",check:"왼쪽 LIVE MVP에서 버튼·입력·결과가 실제로 작동해야 1단계가 끝납니다."},
...
function complete(i){if(i===0)return["name","user","problem","outcome","required"].every(k=>clean(state.mvp[k]))&&hasRunnableHtml();if(i===1)return["issue","must","mustNot","askAgain"].every(k=>clean(state.refine[k]))&&hasRunnableHtml();if(i===2)return clean(state.validate.userResult)&&clean(state.validate.aiResult);return true}
...
function codeArea(label){const status=hasRunnableHtml()?"실행 가능한 HTML이 감지되었습니다. 왼쪽 LIVE MVP에 적용됩니다.":"AI가 준 HTML 전체 코드를 여기에 붙여넣으세요. ```html 코드펜스가 있어도 자동 제거합니다.";return`<div class="field full"><label for="artifactHtml">${label}</label><textarea id="artifactHtml" class="codebox" spellcheck="false" placeholder="<!doctype html> ...">${esc(state.artifactHtml)}</textarea><p class="code-status">${status}</p></div>`}
...
function render(){renderProgress();renderHeader();renderLive();renderForm();renderAI();$("#prev").disabled=state.stage===0;$("#next").textContent=state.stage===3?"다음 개발 단계 확인":"다음 단계";$("#next").disabled=state.stage<3&&!complete(state.stage)}
$("#copyAI").onclick=copyPrompt;$("#prev").onclick=()=>{if(state.stage>0){state.stage--;save();render()}};$("#next").onclick=()=>{if(state.stage<3){if(!complete(state.stage)){alert("이 단계의 실행 기준을 먼저 충족해 주세요.");return}state.stage++;save();render()}else alert(`현재 판단: ${architecture()}\n여기서 다음 개발 범위를 정합니다.`)};$("#reset").onclick=()=>{if(confirm("현재 대상의 작업 내용을 모두 지울까요?")){localStorage.removeItem(KEY);state=init();render()}};
load();render();
})();