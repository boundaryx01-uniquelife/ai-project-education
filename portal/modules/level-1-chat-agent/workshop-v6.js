...
function init(){return{stage:0,mvp:{name:profile.name,user:"",problem:"",outcome:"",required:"",rules:""},selfCheck:{inputChanges:false,ruleBased:false,notEcho:false},artifactHtml:"",refine:{issue:"",change:"",constraintConfirmed:false},refinementHistory:[],lastRefinement:"",validate:{testSituation:"",testOutcome:"",feedback:"",aiMode:"skip",aiResponse:"",accepted:"",rejected:"",decisionReason:"",skipReason:"",substituteEvidence:"",revisionNeeded:"",revisionPlan:"",revisionApplied:false,retestSituation:"",retestOutcome:"",retestFeedback:"",retestPassed:false},validationHistory:[],lastValidation:"",delivery:{place:"local",share:"personal",browserStorage:"no",database:"no",external:"no",nextFunctions:""}}}
let state=init();
function load(){try{const raw=localStorage.getItem(KEY);if(raw){const s=JSON.parse(raw);state={...init(),...s,mvp:{...init().mvp,...s.mvp},selfCheck:{...init().selfCheck,...s.selfCheck},refine:{...init().refine,...s.refine},validate:{...init().validate,...s.validate},refinementHistory:Array.isArray(s.refinementHistory)?s.refinementHistory:[],validationHistory:Array.isArray(s.validationHistory)?s.validationHistory:[],delivery:{...init().delivery,...s.delivery}}}}catch(_){} }
...
function archiveValidation(){const v=state.validate;const record={testSituation:clean(v.testSituation),testOutcome:clean(v.testOutcome),feedback:clean(v.feedback),aiMode:v.aiMode,accepted:clean(v.accepted),rejected:clean(v.rejected),decisionReason:clean(v.decisionReason),skipReason:clean(v.skipReason),substituteEvidence:clean(v.substituteEvidence),revisionNeeded:v.revisionNeeded,revisionPlan:clean(v.revisionPlan),retestSituation:clean(v.retestSituation),retestOutcome:clean(v.retestOutcome),retestFeedback:clean(v.retestFeedback),retestPassed:v.retestPassed};const fingerprint=JSON.stringify(record);if(fingerprint!==state.lastValidation){state.validationHistory.push(record);state.lastValidation=fingerprint}}
...
function codeArea(label,stageThree=false){const status=hasRunnableHtml()?"실행 가능한 HTML이 감지되었습니다. 왼쪽 LIVE MVP에 적용됩니다. 실제 동작을 직접 확인하세요.":"AI가 준 HTML 전체 코드를 여기에 붙여넣으세요. ```html 코드펜스가 있어도 자동 제거합니다.";const confirm=stageThree?check("validate.revisionApplied","수정 HTML을 붙여넣고 왼쪽 LIVE MVP에서 검증 결과가 반영됐는지 확인했다.",state.validate.revisionApplied):"";return`<div class="field full"><label for="artifactHtml">${label}</label><textarea id="artifactHtml" class="codebox" spellcheck="false" placeholder="<!doctype html> ...">${esc(state.artifactHtml)}</textarea><p class="code-status">${status}</p>${confirm}</div>`}
...
function renderValidationForm(){const v=state.validate;const reflection=v.revisionNeeded==="yes"?`<section class="validation-loop full"><strong>검증 결과 반영 · 같은 화면에서 수정합니다</strong><p>오른쪽의 ‘AI에 보낼 내용 복사’에는 검증 결과와 현재 HTML이 정리됩니다. AI가 준 <b>완전한 HTML</b>을 아래에 붙여넣으면 왼쪽 LIVE MVP가 바로 교체됩니다.</p>${field("반영할 수정 내용","validate.revisionPlan",v.revisionPlan,true)}${codeArea("검증 결과를 반영한 수정 HTML 전체 코드",true)}<div class="retest-grid">${field("수정 후 재검증 상황","validate.retestSituation",v.retestSituation,true)}${field("수정 후 실제 결과","validate.retestOutcome",v.retestOutcome,true)}${field("수정 후 피드백","validate.retestFeedback",v.retestFeedback,true)}${check("validate.retestPassed","수정한 MVP가 이번 재검증을 통과했다.",v.retestPassed)}</div></section>`:"";$("#webForm").innerHTML=`<div class="validation-grid">${field("실제 테스트 또는 사용 상황","validate.testSituation",v.testSituation,true,3)}${field("실제로 일어난 일","validate.testOutcome",v.testOutcome,true,3)}${field("발견한 문제 또는 피드백","validate.feedback",v.feedback,true,3)}${select("validate.aiMode","AI 검증 선택",v.aiMode,[["skip","생략하고 대체 검증 기록"],["use","외부 AI로 검증"]])}<p class="validation-message full"><strong>AI 출력도 검증 대상입니다.</strong> 붙여넣은 답보다 학습자의 채택·기각 판단이 중요합니다.</p>${v.aiMode==="use"?`${field("외부 AI 검증 응답","validate.aiResponse",v.aiResponse,true,4)}${field("채택한 제안 (없으면 '없음'과 이유)","validate.accepted",v.accepted,true)}${field("기각한 제안 (해당하는 경우)","validate.rejected",v.rejected,true)}${field("채택·기각 판단 이유","validate.decisionReason",v.decisionReason,true)}`:`${field("AI 검증을 생략한 이유","validate.skipReason",v.skipReason,true)}${field("대신 사용한 검증 방법 또는 근거","validate.substituteEvidence",v.substituteEvidence,true)}`}<div class="validation-loop full"><strong>검증 결과를 최종 HTML에 반영할까요?</strong>${select("validate.revisionNeeded","반영 여부",v.revisionNeeded,[["","선택하세요"],["yes","예 — 이 화면에서 수정 HTML을 반영하고 재검증"],["no","아니오 — 현재 MVP는 이 검증을 통과함"]])}</div>${reflection}</div>`;bindInputs()}
...
function projectMarkdown(){const m=state.mvp,d=state.delivery;const refinement=historyLines(state.refinementHistory,(r,i)=>`${i+1}. 발견한 문제: ${r.issue}\n   - 행동·제약 변경: ${r.change}`,"기록 없음");const validation=historyLines(state.validationHistory,(v,i)=>`${i+1}. 테스트 상황: ${v.testSituation}\n   - 실제 결과: ${v.testOutcome}\n   - 문제·피드백: ${v.feedback}\n   - AI 검증: ${v.aiMode==="use"?`실시 / 채택: ${v.accepted} / 기각: ${v.rejected||"해당 없음"} / 이유: ${v.decisionReason}`:`생략 / 이유: ${v.skipReason} / 대체 근거: ${v.substituteEvidence}`}\n   - 수정 반영: ${v.revisionNeeded==="yes"?`필요 / ${v.revisionPlan}`:"현재 MVP 통과"}`,"기록 없음");return`# ${clean(m.name)||"LEVEL 1 프로젝트"} 학습 패키지
