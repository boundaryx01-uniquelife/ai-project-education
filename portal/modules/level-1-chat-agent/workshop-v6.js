...
$("#copyAI").onclick=copyPrompt;$("#prev").onclick=()=>{if(state.stage>0){state.stage--;save();render()}};$("#next").onclick=()=>{const msg=gateMessage(state.stage);if(msg){alert(msg);return}if(state.stage===1){archiveRefinement();state.stage=2}else if(state.stage===2){archiveValidation();if(state.validate.revisionNeeded==="yes"){state.refine={...init().refine,issue:state.validate.feedback,change:state.validate.revisionPlan};state.validate=init().validate;state.stage=1}else state.stage=3}else if(state.stage<3)state.stage++;else{alert(`현재 판단: ${architecture()}\n다음 개발 경계가 기록되었습니다. 프로젝트 패키지를 내보내세요.`);return}save();render()};$("#reset").onclick=()=>{if(confirm("현재 대상의 작업 내용을 모두 지울까요?")){localStorage.removeItem(KEY);state=init();render()}};
